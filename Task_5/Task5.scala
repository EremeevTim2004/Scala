...
@main def task5_1(): Unit = {
  // ----TASK 1
  def insertionSort[T](list: List[T])(using ord: Ordering[T]): List[T] =
    list match {
      case Nil          => Nil
      case head :: tail => insert(head, insertionSort(tail))
    }

  def insert[T](elem: T, sortedList: List[T])(using ord: Ordering[T]): List[T] =
    sortedList match {
      case Nil => List(elem)
      case head :: tail =>
  ...
  def kthStatistic[T](k: Int, list: List[T])(using
      ord: Ordering[T]
  ): Option[T] = {
