...
@main def task4_1(): Unit = {
  ...
  // ----TASK1b
  val weightChipsC = (
      (potato: BigInt, waterPotato: Double, waterChips: Double) =>
        (potato.toDouble * (1 - waterPotato) / (1 - waterChips)).formatted(
          "%.2f"
        )
  ).curried

  // ----TASK1c
  def weightChipsC1: BigInt => Double => Double => String = potato =>
    waterPotato =>
      waterChips =>
        (potato.toDouble * (1 - waterPotato) / (1 - waterChips))
          .formatted("%.2f")

  // ----TASK2a
  def strToColDigits: String => Int = str => str.count(_.isDigit)

  def strToSumDigits: String => Int = str =>
    str.filter(_.isDigit).map(_.asDigit).sum
  ...
  val performOperation: PartialFunction[String, (Double, Double) => Double] = {
    ...
    case "/" =>
      (x, y) =>
        if (y != 0) x / y else throw new ArithmeticException("/ by zero")
  }
  ...
  try {
    ...
    case a: ArithmeticException =>
      println(s"ArithmeticException: ${a.getMessage}")
    case b: IllegalArgumentException =>
      println(s"IllegalArgumentException: ${b.getMessage}")
    case c: NumberFormatException =>
      println(s"NumberFormatException: ${c.getMessage}")
  }
}
